...
% =========
%
% Adapted from ROB501 A2, by T.D Barfoot, January 2016
%
clear all;
...
% ================================================
% load the occupancy map from file
% ================================================
...
% =========================================================================
% localization from an occupancy grid map using particle filter
% =========================================================================
%
...
% 
            ...
            R_wr = [
                cos(theta) -sin(theta) x_interp(i);
                sin(theta) cos(theta) y_interp(i);
                0 0 1];
...
%           laser = [cur_laser*cos(angle(j))-0.1; cur_laser*sin(angle(j)); 1];
            laser = R_wr*laser;
            
            prior_x_coord = round((laser(1)-ogxmin)/ogres);
            prior_y_coord = round((laser(2)-ogymin)/ogres);
            
            %get predicted posterior
            pred_pt = T*[cos(angles(j)), -sin(angles(j)), -0.1; sin(angles(j)), cos(angles(j)), 0; 0, 0, 1];
            pred_x = round((pred_pt(1,3) - ogxmin)/ogres);
            pred_y = round((pred_pt(2,3) - ogymin)/ogres);
            pred_theta = atan2(pred_pt(2,1), pred_pt(1,1));
            
            pred_x = max(pred_x, 1);
            pred_x = min(pred_x, ognx);
            pred_y = max(pred_y, 1);
            pred_y = min(pred_y, ogny);
            
            
            for inc=0.45:ogres:5
                
                if oglo(round(pred_y), round(pred_x)) >= 0
                    continue;
                end
                
                pred_x = pred_x + cos(pred_theta)*inc;
                pred_y = pred_y + sin(pred_theta)*inc;
                
                pred_x = max(pred_x, 1);
                pred_x = min(pred_x, 300);
                pred_y = max(pred_y, 1);
                pred_y = min(pred_y, 180);
            end
            
            e = norm([pred_x;pred_y]-[prior_x_coord;prior_y_coord]);
            w_particle(n) = w_particle(n) + w_gain*(1/sqrt(2*pi*laser_var)*exp(-e^2/(2*laser_var)));
